...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'update_foto') {
        ...
        if (!empty($foto_data)) {
            ...
            $stmt->bind_param("si", $foto_data, $user_id);
            if ($stmt->execute()) {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'update_akun') {
        ...
        if (!empty($password)) {
            ...
            $stmt->bind_param("sssi", $nama, $username, $hashed, $user_id);
        } else {
...
// Fetch User Data
$user_data = $conn->query("SELECT nama, username, foto_profil FROM users WHERE id = $user_id")->fetch_assoc();
...
<head>
    ...
    <title>Kelola Akun - ROHIS Portal</title>
...
<body class="text-slate-800 antialiased selection:bg-blue-500 selection:text-white">
    <div class="flex min-h-screen">
        ...
        <aside class="w-64 bg-white border-r border-slate-200 flex flex-col fixed h-full z-20 shadow-sm">
            <div class="h-20 flex items-center px-6 border-b border-slate-100">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs font-medium text-slate-500">Portal Absensi</p>
                    </div>
                </div>
            </div>
            ...
            <div class="px-6 py-5 border-b border-slate-100">
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-bold text-slate-800 truncate"><?= htmlspecialchars($user_nama) ?></p>
            ...
            <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
                ...
                <a href="home.php" class="sidebar-item w-full flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium text-slate-600 text-left">
                    <i data-lucide="layout-dashboard" class="icon w-5 h-5 text-slate-400"></i>
                    Dashboard
                </a>
                <a href="akun.php" class="sidebar-item active w-full flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium text-slate-600 text-left">
        ...
        <main class="flex-1 ml-64 p-8">
            ...
            <!-- Messages -->
            <?php if ($msg): ?>
            <div class="mb-6 p-4 rounded-xl border flex items-start gap-3 <?= $msg_type === 'success' ? 'bg-green-50 border-green-200 text-green-800' : 'bg-red-50 border-red-200 text-red-800' ?> text-sm font-medium">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden relative">
                    ...
                    <div class="p-8 pt-0 relative">
                        ...
                        <div class="flex items-end gap-5 -mt-12 mb-8">
                            <div class="profile-avatar rounded-xl border-4 border-white shadow-sm overflow-hidden flex-shrink-0 bg-blue-50" style="width:110px;height:110px;min-width:110px;" onclick="showPhotoOptions()">
                                <?php if (!empty($foto_profil)): ?>
                                    <img id="avatarPreview" src="<?= htmlspecialchars($foto_profil) ?>" class="w-full h-full object-cover">
                                <?php else: ?>
                                    <div id="avatarPreview" class="w-full h-full flex items-center justify-center text-blue-600 text-4xl font-black">
                            ...
                            <div class="pb-2">
                                <h3 class="text-xl font-extrabold text-slate-800"><?= htmlspecialchars($user_data['nama'] ?? 'User') ?></h3>
                        ...
                        <form method="POST" class="space-y-5">
                            ...
                            <div class="space-y-1.5">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" name="nama" value="<?= htmlspecialchars($user_data['nama'] ?? '') ?>" required
                                        class="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-md text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all">
                                </div>
                            </div>
                            
                            <div class="space-y-1.5">
                                <label class="block text-sm font-medium text-slate-500">Username</label>
                                <div class="relative">
                            ...
                            <div class="space-y-1.5">
                                <label class="block text-sm font-medium text-slate-500">Password Baru</label>
                                <div class="relative">
                            ...
                            <div class="pt-4 mt-6 border-t border-slate-100 flex justify-end">
                                <button type="submit" class="px-6 py-3 bg-rohis-600 hover:bg-rohis-700 text-white rounded-md shadow-sm transition-all flex items-center gap-2 font-bold text-sm">
    ...
    <!-- Photo Options Modal -->
    <div id="photoOptionsModal" class="fixed inset-0 z-50 hidden items-center justify-center p-4" style="background:rgba(15,23,42,0.6);">
        <div class="bg-white rounded-xl shadow-lg max-w-sm w-full overflow-hidden border border-slate-100" id="photoOptionsContent" style="transform:scale(0.95);transition:transform 0.2s ease;">
            ...
            <div class="p-5 space-y-3">
                <!-- Pilih dari Galeri -->
                <button type="button" onclick="pickFromGallery()" class="w-full flex items-center gap-4 p-4 rounded-xl border border-slate-200 hover:border-blue-300 hover:bg-blue-50/50 transition-all group">
                    <div class="w-12 h-12 rounded-md bg-blue-50 text-blue-600 flex items-center justify-center group-hover:bg-blue-100 transition-colors">
                ...
                <!-- Ambil Selfie -->
                <button type="button" onclick="openCamera()" class="w-full flex items-center gap-4 p-4 rounded-xl border border-slate-200 hover:border-blue-300 hover:bg-blue-50/50 transition-all group">
                    <div class="w-12 h-12 rounded-md bg-green-50 text-green-600 flex items-center justify-center group-hover:bg-green-100 transition-colors">
                ...
                <?php if (!empty($foto_profil)): ?>
                <button type="button" onclick="deletePhoto()" class="w-full flex items-center gap-4 p-4 rounded-xl border border-red-200 hover:border-red-300 hover:bg-red-50/50 transition-all group">
                    <div class="w-12 h-12 rounded-md bg-red-50 text-red-500 flex items-center justify-center group-hover:bg-red-100 transition-colors">
    ...
    <!-- Camera Modal -->
    <div id="cameraModal" class="fixed inset-0 z-[60] hidden items-center justify-center p-4" style="background:rgba(15,23,42,0.6);">
        <div class="bg-white rounded-xl shadow-lg max-w-md w-full overflow-hidden border border-slate-100">
            <div class="flex justify-between items-center p-5 border-b border-slate-100">
                <div class="flex items-center gap-2 text-slate-800">
            ...
            <div class="p-5">
                <div class="rounded-xl overflow-hidden bg-black mb-4 relative" style="aspect-ratio:1/1;">
                    <video id="cameraVideo" autoplay playsinline class="w-full h-full object-cover"></video>
                    <canvas id="cameraCanvas" class="hidden"></canvas>
                </div>
                <button type="button" onclick="capturePhoto()" class="w-full py-3 bg-rohis-600 hover:bg-rohis-700 text-white rounded-md font-bold text-sm flex items-center justify-center gap-2 shadow-sm">
